<script lang="ts">
  ...
  const NAV_LINKS: { href: string; text: string }[] = [
    { text: 'my stack', href: '/tech-stack' },
    { text: 'resume', href: '/resume' },
  ];
  const ICONS_CLASSES: { href: string; classes: string }[] = [
    {
      classes: 'fa-solid fa-envelope',
      href: 'mailto:gabriele.filaferro@gmail.com',
    },
    {
      classes: 'fa-brands fa-instagram-square',
      href: 'https://www.instagram.com/gabri.fila/',
    },
    {
      classes: 'fa-brands fa-linkedin',
      href: 'https://www.linkedin.com/in/gabriele-filaferro/',
    },
  ...
  onMount(() => {
    ...
    const CSS_ROOT = document?.querySelector(':root') as
      | HTMLElement
      | undefined;
...
>
  ...
  >
    <div class="flex justify-between flex-1 w-full items-center">
      <a href="/">
        <h1 class="capitalize font-bold text-3xl sm:text-5xl grad-text">
          Gabriele Filaferro
        </h1>
      ...
      <i
        on:click={toggleMobileMenu}
        class="fa-solid fa-bars sm:hidden text-secondary text-3xl"
      />
    </div>
  ...
  <div class="flex justify-between items-end pointer-events-auto p-4 sm:p-0">
    <a
      href="https://github.com/GabriFila/GabriFila"
      target="_blank"
      class="flex items-center gap-2 text-textDisabled hover:text-text"
    >
      <p class="hidden sm:block  text-xs sm:text-lg whitespace-nowrap ">
        Built with SvelteKit
      </p>
      <i class="fa-solid fa-up-right-from-square text-base" />
    </a>
      ...
      >
        {#each ICONS_CLASSES as { classes, href }}
          <a {href} target="_blank">
            <i
              class={`${classes} text-2xl sm:text-3xl text-textDisabled pointer-coarse:text-text hover:text-text grow-on-hover`}
            />
          </a>
        {/each}
      </div>
